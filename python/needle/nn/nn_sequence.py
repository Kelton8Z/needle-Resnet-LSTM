...
class RNNCell(Module):
    ...
    def forward(self, X, h=None):
        ...
        if self.bias:
            ih = X @ self.W_ih + self.bias_ih.reshape((1, self.hidden_size)).broadcast_to((batch_size, self.hidden_size))
            hh = h @ self.W_hh + self.bias_hh.reshape((1, self.hidden_size)).broadcast_to((batch_size, self.hidden_size))
            return self.nonlinearity(ih + hh)
        else:
